...
6. [Deferred: merge belongs to shipping stage] **Merge branch to main and verify integration** — S
...
- [x] Lint passes (`make validate`).
- [x] Tests pass; `tests/test_orchestrator.py` covers run_single hygiene (halt + unattended-warn), the plan-presence guard, and run-summary emission on success/failure. `check_main_hygiene` itself remains covered by `tests/test_worktree.py`.
- [x] `run_single` reaches the shared hygiene gate (grep confirms it is called from both `run_single` and `run_batch`).
- [x] `aet-work report` shows a `run_summary` row after a run-one invocation (telemetry no longer empty).
- [x] `aet-work/SKILL.md` no longer pipes the orchestrator through `tee`.
- [ ] Merge verified: `git merge-base --is-ancestor HEAD origin/main`.

## Divergence Summary

_Recorded: 2026-06-18 — Branch: 2026-06-18-orchestrator-run-one-hardening_

### Deferred

- **Task 6 (Merge branch to main and verify integration):** Deferred to the shipping stage (`aet-ship`). The implementation pipeline (`aet-tdd → aet-implement → aet-qa → aet-review`) stops at `reviewed`; merging to `main` is out of scope for this stage.
