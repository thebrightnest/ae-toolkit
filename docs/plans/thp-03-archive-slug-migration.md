...
- The rename is executed manually by the owner against the live `~/.aet` archive; the script ships in-repo (precedent: `scripts/archive/migrate-plans-to-frontmatter.py`) so QA exercises it against a fixture archive, never the real one.
...
- `scripts/archive/migrate-telemetry-slugs.py OLD_SLUG NEW_SLUG [--apply] [--archive PATH] [--reports PATH]`
...
1. ✓ Write `scripts/archive/migrate-telemetry-slugs.py` per Locked design (argparse, dry-run default, containment validation, collision refusal, idempotency) — M (traces: R-3)
...
- `scripts/archive/migrate-telemetry-slugs.py` (new)
