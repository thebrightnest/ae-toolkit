...
- **Work-class terminology:** `aet-ship` checks for `*Work class: critical*` (the value used in existing plan footers and the triage-front-door PRD), not `critical-class` (the wording in `clv-02-foundation-smoke-integration.md`). This keeps the gate consistent with the routing convention already in use.
- **aet-setup line budget:** Condensed the existing AGENTS.md subsection into a tighter paragraph so the new Smoke Checks and Validation Calibration sections fit under the 400-line SKILL.md limit. No semantic content was removed.
- **`make validate` scope:** Repo-wide `make validate` fails on pre-existing untracked plan/PRD files from parallel work. The modified skill files individually pass all quality gates.

### Deferred

- **aet-verify skill directory:** The aet-ship gate documents the evidence convention (`.agents/verify/{ticket}-evidence.md`) but the actual `aet-verify` skill directory does not exist on this branch. That work is tracked in `clv-01-aet-verify-skill.md` (stage `synced` on branch `clv-01`).
