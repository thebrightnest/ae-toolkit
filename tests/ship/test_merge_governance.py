...
AET_WORK_COMMANDS = REPO_ROOT / ".agents" / "commands" / "aet-work.md"
...
class TestAutonomousMergeGovernance:
    ...
    def test_aet_ship_commands_state_human_merge_boundary(self):
        """The canonical aet-work command doc must state that the human decides to merge."""
        content = AET_WORK_COMMANDS.read_text()
        lower = content.lower()
        assert "merge is the human's decision" in lower
        assert "trying to merge the pr yourself" in lower
