...
approved, queued, in-progress, awaiting-merge, and closed.

Workflow stage names map to board columns positionally from the loaded
workflow (entry → approved, terminal skill-less → queued, anything else →
in-progress), so variant vocabularies render a sensible board without a
per-workflow mapping table.
"""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Review plans grouped by pipeline stage."
    )
    parser.add_argument(
        "--plans-dir",
        default="docs/plans",
