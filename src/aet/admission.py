...
def settled_ids_from(
    ...
    if backend is not None:
        ...
        if getter is not None:
            ...
            try:
                data = backend.load()
                settled.update(t.get("id") for t in data.get("history", []) if t.get("id"))
            except Exception:
                pass

    path = history_file or (getattr(backend, "history_file", None) if backend else None)
    if path and Path(path).exists():
        try:
            settled |= {
                record["id"]
                for record in read_history(str(path))
                if record.get("id") and record.get("state") in HISTORY_TERMINAL_STATES
            }
        except Exception:
            pass
...
def unacked_intake_findings(
    ...
    settled_ids = settled_ids_from(backend=backend, history_file=history_file, history=history)
...
def admit_plan(
    ...
    if plan_stem in live_ids or any(t.get("plan_file") == str(plan_file) for t in live_tasks):
        existing = next(
            (t for t in live_tasks if t.get("id") == plan_stem or t.get("plan_file") == str(plan_file)),
            None,
        )
        return Skipped(
            reason=SkipReason.ALREADY_IN_QUEUE,
    ...
    if not allow_blocked and task.get("state") == "blocked":
        blockers = [
            ...
            if b in live_ids
            and next(
                (t for t in live_tasks if t.get("id") == b and t.get("state") in {"merged", "abandoned"}),
                None,
            )
            is None
        ]
