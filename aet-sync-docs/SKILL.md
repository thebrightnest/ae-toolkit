...
2. Determine the output path with this precedence:
   ...
   2. `$AET_EVIDENCE_PATH_SYNC_DOCS` if set (group sessions publish one per stage).
   3. Default: `~/.aet/reports/{project-slug}/{task-id}/sync-docs.json`.
      When `aet-work/lib` is importable, call `resolve_verdict_path(task_id, "sync-docs")` from `aet-work/lib/evidence.py` — the canonical helper implementing this precedence. Never hand-compute `{project-slug}` from the worktree CWD.
3. Use `aet-work/lib/evidence.py` (`write_verdict`) when available; otherwise write equivalent JSON to the resolved path.
4. Only update the plan.md footer to `*Stage: synced*` after the verdict file is written.
